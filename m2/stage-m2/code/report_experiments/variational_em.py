...
class VariationalEM():
    ...
    def ELBO(self):
        ...
        # MADE CHANGES HERE
        result1 = np.dot(np.dot(self.A,self.tau),np.log(self.Gamma))
        result2 = np.dot(np.dot((1-np.eye(self.n)-self.A), self.tau), np.log(1-self.Gamma))
        ELBO =\
                -np.sum(self.tau*np.log(self.tau))\
                + np.sum(np.dot(self.tau, np.log(self.Pi)))\
                + .5*np.sum(np.multiply(self.tau,result1))\
                + .5*np.sum(np.multiply(self.tau,result2))
    ...
    def E_step(self):
        ...
        def fixed_point(self, tau):
            ...
            # MADE CHANGES HERE
            aux_matrix = np.matlib.repmat(np.max(lik, axis=1), 1, self.k)
            unnorm = np.exp(lik - aux_matrix)  # substracting the max is numerical trick
